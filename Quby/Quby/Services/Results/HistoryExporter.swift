...
struct HistoryExporter {
    ...
    func writeCSV(_ records: [CodeRecord]) -> URL? {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("Quby codes.csv")
        do {
    ...
    func writeText(_ records: [CodeRecord]) -> URL? {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("Quby codes.txt")
        do {
